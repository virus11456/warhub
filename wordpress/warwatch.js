    ...
    if (status.allClosed) {
      ...
      if (badge) {
        ...
        const isDST = (() => {
          ...
          const dstStart = new Date(Date.UTC(y,2,8+(7-new Date(Date.UTC(y,2,8)).getUTCDay())%7));
          const dstEnd   = new Date(Date.UTC(y,10,1+(7-new Date(Date.UTC(y,10,1)).getUTCDay())%7));
        ...
        if (etHour >= 11 && etHour < 22) {
          badge.textContent = `🍕 訂單中（ET ${h12}:${etMin} ${period}）`;
          badge.style.borderColor = badge.style.color = 'var(--ww-g)';
...
<div style="color:var(--ww-dim)">▶ 若再有 1 家店升至異常，將觸發紅色戰爭警報</div>`;
  ...
  function recalcWpi() {
    ...
    // ── 回寫 WPI 到 PHP（供歷史記錄）────────────────────────
    fetch(`${window.wwSiteUrl}/wp-json/warwatch/v1/wpi-update`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ wpi }),
    }).catch(() => {});

    // ── 更新相關性圖表 ────────────────────────────────────
    if (typeof renderCorrChart === 'function') renderCorrChart();
  ...
  <div class="ww-poly-item ${cls}" style="cursor:pointer;display:flex;flex-direction:column;justify-content:space-between" onmouseover="this.style.borderLeftWidth='5px'" onmouseout="this.style.borderLeftWidth='3px'">
    ...
    <div class="ww-poly-meta">
      ...
      list2.innerHTML = STATIC_POLY.map(m => {
        ...
        const color = m.color;
        const cls   = pct >= 70 ? 'danger' : pct >= 40 ? 'warn' : '';
        return `<div class="ww-poly-item ${cls}" style="display:flex;flex-direction:column;justify-content:space-between">
    <div class="ww-poly-q">${m.question}</div>
    <div class="ww-poly-bar-row">
      <span style="font-family:'Share Tech Mono',monospace;font-size:.62rem;color:var(--ww-dim);width:24px">YES</span>
      <div class="ww-poly-bar"><div class="ww-poly-fill" style="width:${pct}%;background:linear-gradient(90deg,#001,${color})"></div></div>
  ...
  ready(function () {
    ...
    // ── 相關性圖表：載入後立即抓，之後每 10 分鐘更新 ────────
    fetchCorrHistory();
    setInterval(fetchCorrHistory, 10 * 60 * 1000);

    // ── RSS 新聞：載入後立即抓，之後每 30 分鐘更新 ──────────
    fetchRssFeed();
  ...
  // 計算並渲染相關係數標籤
  function renderCorrBadges(history) {
    const hasChg = history.filter(h => h.wpi > 0 && h.gold_chg != null && h.gold_chg !== 0);
    const valid  = hasChg.length >= 5 ? hasChg : history.filter(h => h.wpi > 0 && h.gold_price != null);
    if (valid.length < 3) return;
    const useChg = hasChg.length >= 5;
    const wpis  = valid.map(h => h.wpi);
    const golds = valid.map(h => useChg ? h.gold_chg  : h.gold_price);
    const oils  = valid.map(h => useChg ? h.oil_chg   : h.oil_price);
    const prems = valid.map(h => h.war_prem);
    const vixValid = history.filter(h => h.wpi > 0 && h.vix_price != null);
    const chfValid = history.filter(h => h.wpi > 0 && h.chf_price != null);
    const rVix  = vixValid.length >= 3 ? pearson(vixValid.map(h=>h.wpi), vixValid.map(h=>h.vix_price)) : null;
    const rChf  = chfValid.length >= 3 ? pearson(chfValid.map(h=>h.wpi), chfValid.map(h=>h.chf_price)) : null;
    const rGold = pearson(wpis, golds);
    const rOil  = pearson(wpis, oils);
    const rPrem = pearson(wpis, prems);
    const fmt = r => {
      if (r === null) return { txt: 'N/A', color: '#4a6070' };
      const abs = Math.abs(r), dir = r >= 0 ? '↑' : '↓';
      const color = abs >= 0.7 ? '#ff4444' : abs >= 0.4 ? '#ff8800' : '#4a6070';
      const lbl   = abs >= 0.7 ? '強相關' : abs >= 0.4 ? '中度相關' : '弱相關';
      return { txt: `${dir} ${r.toFixed(3)} (${lbl})`, color };
    };
    const setCorr = (id, val) => {
      const el = document.getElementById(id); if (!el) return;
      const f = fmt(val); el.textContent = f.txt; el.style.color = f.color;
    };
    setCorr('corr-gold', rGold);
    setCorr('corr-oil',  rOil);
    setCorr('corr-prem', rPrem);
    setCorr('corr-vix',  rVix);
    setCorr('corr-chf',  rChf);
    const lastEntry = history[history.length - 1];
    const anomalyEl = document.getElementById('corr-anomaly');
    if (anomalyEl && lastEntry) {
      const zScores = [
        { label: '黃金', z: lastEntry.gold_z },
        { label: 'VIX',  z: lastEntry.vix_z  },
        { label: '瑞郎',  z: lastEntry.chf_z  },
        { label: '油價',  z: lastEntry.oil_z  },
      ].filter(x => x.z != null && Math.abs(x.z) > 2);
      if (zScores.length >= 2) {
        const labels = zScores.map(x=>`${x.label}(${x.z>0?'+':''}${x.z}σ)`).join(' · ');
        anomalyEl.textContent = `⚠ 多重異常信號：${labels}`;
        anomalyEl.style.display = 'block';
        anomalyEl.style.color = zScores.length >= 3 ? '#ff4444' : '#ff8800';
      } else if (zScores.length === 1) {
        anomalyEl.textContent = `觀察：${zScores[0].label} Z=${zScores[0].z}σ`;
        anomalyEl.style.display = 'block'; anomalyEl.style.color = '#c8d8e0';
      } else { anomalyEl.style.display = 'none'; }
    }
    const modeEl = document.getElementById('corr-mode');
    if (modeEl) modeEl.textContent = useChg ? '（漲跌幅相關）' : '（價格相關）';
    const cnt = document.getElementById('corr-sample-count');
    if (cnt) cnt.textContent = `樣本數：${valid.length} 筆`;
  }
  window.renderCorrChart = function(retry) {
    if (!corrHistoryData.length) return;
    if (typeof Chart === 'undefined') {
      if ((retry||0) < 10) setTimeout(function(){ window.renderCorrChart((retry||0)+1); }, 500);
      return;
    }
    const canvas = document.getElementById('corr-chart');
    if (!canvas) return;

    const valid = corrHistoryData.filter(h => h.wpi > 0 && h.gold_price != null);
    if (valid.length < 2) return;
    const loading = document.getElementById('corr-loading');
    if (loading) loading.style.display = 'none';
    canvas.style.display = 'block';
    const labels  = valid.map(h => h.ts.slice(5,16));
    const wpiData = valid.map(h => h.wpi);
    function zscore(arr) {
      return arr.map((v,i) => {
        const win=arr.slice(Math.max(0,i-19),i+1).filter(x=>x!=null);
        if(win.length<5||v==null) return null;
        const mean=win.reduce((a,b)=>a+b,0)/win.length;
        const std=Math.sqrt(win.reduce((s,x)=>s+Math.pow(x-mean,2),0)/win.length);
        return std<0.0001?null:parseFloat(((v-mean)/std).toFixed(2));
      });
    }
    const goldZ=zscore(valid.map(h=>h.gold_price));
    const vixZ =zscore(valid.map(h=>h.vix_price));
    const chfZ =zscore(valid.map(h=>h.chf_price?-h.chf_price:null));
    const oilZ =zscore(valid.map(h=>h.oil_price));
    if(corrChartInstance) corrChartInstance.destroy();
    corrChartInstance=new Chart(canvas,{
      type:'line',
      data:{labels,datasets:[
        {label:'WPI 戰爭壓力',data:wpiData,borderColor:'#ff4444',backgroundColor:'rgba(255,68,68,0.06)',borderWidth:2.5,pointRadius:2,yAxisID:'y',tension:0.3},
        {label:'黃金 Z-score',data:goldZ,borderColor:'#ffaa00',backgroundColor:'rgba(255,170,0,0.04)',borderWidth:1.5,pointRadius:1,yAxisID:'yz',tension:0.3},
        {label:'VIX Z-score',data:vixZ,borderColor:'#ff6600',backgroundColor:'rgba(255,102,0,0.04)',borderWidth:1.5,pointRadius:1,yAxisID:'yz',tension:0.3},
        {label:'瑞郎強度 Z-score',data:chfZ,borderColor:'#00ff88',backgroundColor:'rgba(0,255,136,0.04)',borderWidth:1.5,pointRadius:1,yAxisID:'yz',tension:0.3},
        {label:'油價 Z-score',data:oilZ,borderColor:'#00aaff',backgroundColor:'rgba(0,170,255,0.04)',borderWidth:1,pointRadius:1,yAxisID:'yz',tension:0.3,borderDash:[3,3]},
      ]},
      options:{
        responsive:true,maintainAspectRatio:false,
        interaction:{mode:'index',intersect:false},
        plugins:{
          legend:{labels:{color:'#6a8a9a',font:{family:"'Share Tech Mono',monospace",size:11},boxWidth:14}},
          tooltip:{
            backgroundColor:'rgba(5,10,14,0.95)',borderColor:'#1a3040',borderWidth:1,
            titleColor:'#c8d8e8',bodyColor:'#6a8a9a',
            titleFont:{family:"'Share Tech Mono',monospace"},bodyFont:{family:"'Share Tech Mono',monospace"},
            callbacks:{label:ctx=>{const v=ctx.parsed.y;if(ctx.datasetIndex===0)return ` WPI: ${v}`;if(v==null)return null;return ` ${ctx.dataset.label}: ${v}σ${Math.abs(v)>2?' ⚠':''}`;}}
          },
        },
        scales:{
          x:{ticks:{color:'#6a8a9a',font:{family:"'Share Tech Mono',monospace",size:10},maxTicksLimit:8},grid:{color:'rgba(26,48,64,0.5)'}},
          y:{position:'left',title:{display:true,text:'WPI (0-100)',color:'#ff4444',font:{size:11}},min:0,max:100,ticks:{color:'#ff4444',font:{family:"'Share Tech Mono',monospace",size:10}},grid:{color:'rgba(255,68,68,0.06)'}},
          yz:{position:'right',title:{display:true,text:'Z-score (σ)',color:'#ffaa00',font:{size:11}},min:-4,max:4,ticks:{color:'#6a8a9a',font:{family:"'Share Tech Mono',monospace",size:10},callback:v=>`${v>0?'+':''}${v}σ`},grid:{color:'rgba(255,170,0,0.05)'}},
        },
      },
    });
    renderCorrBadges(corrHistoryData);
  };

  // 從 REST API 抓歷史數據
  async function fetchCorrHistory() {
    if (!window.wwSiteUrl) return;
    try {
      const res = await fetch(`${window.wwSiteUrl}/wp-json/warwatch/v1/corr-history`);
      if (!res.ok) throw new Error(`API ${res.status}`);
      const d = await res.json();
      if (d.history && d.history.length) {
        corrHistoryData = d.history;
        renderCorrBadges(corrHistoryData);
        window.renderCorrChart();
      }
      const cnt = document.getElementById('corr-sample-count');
      if (cnt && d.count) cnt.textContent = `樣本數：${d.count} 筆`;
    } catch(e) {
      console.warn('[WARWATCH] corrHistory fetch failed:', e.message);
    }
  }
  ...
  const WW_SRC_COLOR = {
    ...
    'Al Jazeera':       '#e8a000',
    'DW News':           '#00aacc',
    'Guardian World':   '#0099dd',
    'Euromaidan Press': '#66bb6a',
    'UK Defence Jnl':   '#bb86fc',
  };
  ...
  async function fetchRssFeed() {
    try {
      ...
      const res  = await fetch(`${window.wwSiteUrl}/wp-json/warwatch/v1/rss`, { signal: ctrl.signal });
      clearTimeout(tid);
      if (!res.ok) throw new Error('HTTP ' + res.status);
      const data = await res.json();
      if (!data.items || !data.items.length) return;

      wwRssItems = data.items;
      wwNewsShown = 12;
      wwRenderNews('all');
      wwUpdateTicker(data.items);

      const cnt = document.getElementById('ww-news-count');
      const upd = document.getElementById('ww-news-updated');
      if (cnt) cnt.textContent = data.items.length + ' 則戰情新聞';
      if (upd) upd.textContent = data.updated || '';

      console.log('[WARWATCH] RSS:', data.items.length, '則新聞已載入');
    } catch(e) {
      if (e.name !== 'AbortError') console.warn('[WARWATCH] RSS fetch failed:', e.message);
    }
  }

  function wwRenderNews(src) {
    const grid = document.getElementById('ww-news-grid');
    const btn  = document.getElementById('ww-news-more');
    if (!grid) return;

    const list = (src === 'all' || !src)
      ? wwRssItems
      : wwRssItems.filter(n => n.source === src);

    grid.style.cssText = 'display:grid;grid-template-columns:repeat(3,1fr);gap:.9rem;';
    grid.innerHTML = '';

    if (!list.length) {
      grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:2rem;font-family:\'Share Tech Mono\',monospace;font-size:.7rem;color:#4a6070;">此來源目前無符合的新聞</div>';
      if (btn) btn.style.display = 'none';
      return;
    }

    list.slice(0, wwNewsShown).forEach(function(n) {
      const c  = WW_SRC_COLOR[n.source] || '#1a3040';
      ...
      card.setAttribute('data-src', s);
      card.style.cssText = 'background:#0a121e;border:1px solid #1a3040;border-top:2px solid ' + c + ';padding:.95rem 1.1rem .85rem;display:flex;flex-direction:column;gap:.4rem;cursor:default;transition:background .2s;';
      card.addEventListener('mouseover', function(){ this.style.background='rgba(0,255,136,.03)'; });
      card.addEventListener('mouseout',  function(){ this.style.background='#0a121e'; });
      card.innerHTML =
          '<div style="font-family:monospace;font-size:.58rem;letter-spacing:2px;text-transform:uppercase;color:' + c + ';">' + s + '</div>'
        + '<div style="font-size:.8rem;font-weight:500;color:#c8d8e0;line-height:1.5;flex:1;"><a href="' + lk + '" target="_blank" rel="noopener" style="color:inherit;text-decoration:none;">' + t + '</a></div>'
        + '<div style="display:flex;align-items:center;justify-content:space-between;padding-top:.5rem;border-top:1px solid rgba(26,48,64,.8);margin-top:auto;">'
        + '<span style="font-family:monospace;font-size:.58rem;color:#4a6070;">' + a + '</span>'
        + '<a href="' + lk + '" target="_blank" rel="noopener" style="font-family:monospace;font-size:.58rem;color:#00ff88;text-decoration:none;">→ 閱讀全文</a>'
        + '</div>';
      grid.appendChild(card);
    });

    if (btn) btn.style.display = list.length > wwNewsShown ? 'block' : 'none';
  }

  window.wwNewsShowMore = function() {
    wwNewsShown += 12;
    const active = document.querySelector('.wwnf[data-active="1"]');
    wwRenderNews(active ? active.dataset.src : 'all');
  };

  function wwUpdateTicker(items) {
    const scroll = document.querySelector('.ww-ticker-scroll');
    if (!scroll || !items.length) return;
    const top = items.slice(0, 30).map(n =>
      '<div class="ww-ti">'
      + '<span style="font-family:monospace;font-size:.65rem;color:#00ff88;margin-right:.3rem;opacity:.8;">[' + (n.source||'') + ']</span>'
      + '<a href="' + (n.link||'#') + '" target="_blank" rel="noopener" style="color:inherit;text-decoration:none;">' + (n.title||'') + '</a>'
      + ' <span style="font-family:monospace;font-size:.6rem;color:#4a6070;margin-left:.3rem;">' + (n.age||'') + '</span>'
      + ' &nbsp;•</div>'
    );
    const statics = Array.from(scroll.querySelectorAll('.ww-ti'))
      .filter(el => !el.querySelector('a[href]')).slice(0,6).map(el => el.outerHTML);
    scroll.innerHTML = [...top, ...statics, ...top, ...statics].join('');
    const dur = Math.max(80, (top.length + statics.length) * 4) + 's';
    scroll.style.animationDuration = dur;
  }

  // filter 按鈕事件
  document.addEventListener('click', function(e) {
    const b = e.target.closest('.wwnf');
    if (!b) return;
    document.querySelectorAll('.wwnf').forEach(function(x) {
      x.style.borderColor = '#1a3040';
      x.style.color = '#4a6070';
      x.style.background = '#0a121e';
